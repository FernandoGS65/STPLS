...
var icons = [
            ...
            { href: 'estadisticas.html', page: 'estadisticas.html', label: 'Estad.', svg: '<svg class="nav-icon-svg" viewBox="0 0 24 24"><path d="M16 8v8m-4-5v5m-4-2v2m-2 4h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>' },
            { href: 'arbitros.html', page: 'arbitros.html', label: 'Arbitros', svg: '<svg class="nav-icon-svg" viewBox="0 0 24 24"><path d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>' }
        ];
    ...
    /* ===== FOOTER THEME TOGGLE ===== */
    function renderFooterTheme() {
        var footer = document.querySelector('footer');
        if (!footer) return;
        var existing = footer.querySelector('[data-theme-toggle]');
        if (existing) return;
        var btn = document.createElement('button');
        btn.setAttribute('data-theme-toggle', '');
        btn.className = 'footer-theme-btn';
        var isLight = document.documentElement.classList.contains('light-mode');
        btn.textContent = isLight ? '☀️' : '🌙';
        footer.appendChild(btn);
    }

    /* Override init to also render nav icons and footer theme */
